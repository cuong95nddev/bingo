...
export function computeSumPayoutsDisplay(diceMax: number): Record<number, string> {
  const payouts = computeSumPayouts(diceMax);
  const display: Record<number, string> = {};
  for (const [sum, mult] of Object.entries(payouts)) {
    display[Number(sum)] = `x${mult}`;
  }
  return display;
}
